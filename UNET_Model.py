...
def Model_Unet(Unet_Path, Image_Path, Mask_Path, Predict_Path, model_name='UNET.h5', sol=None):
    ...
    for i in range(image_count):
        ...
        if sol:
            model.fit_generator(myGene, hidden_neuron=round(sol[2]), steps_per_epoch=round(sol[0]),
                                epochs=round(sol[1]), callbacks=[model_checkpoint])
        else:
